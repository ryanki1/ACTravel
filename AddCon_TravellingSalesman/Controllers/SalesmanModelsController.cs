...
namespace AddCon_TravellingSalesman.Controllers
{
    public class SalesmanModelsController : Controller
    {
        ...
        //
        // GET: /SalesmanModels/
        public ViewResult dummySelector()
        {
            var db = context.SalesmanModels;
            // Select top 2 Salesman ordered by surname descending
            var query = (from sm in db
                         orderby sm.LastName
                         select sm).Take(2);
        ...
        [HttpPost]
        public ActionResult Create(SalesmanModel salesmanmodel, string submit)
        {
        ...
        //GET: /SalesmanModels/SequenceSalesman
        public ActionResult SequenceSalesmanJSON(string lat, string lng)
        {
            //var modelByDistance = context.SalesmanModels.ToList().OrderBy(salesman => calculateDistance(lat, lng, salesman.Latitude, salesman.Longitude));
            var modelByDistance = from salesman in context.SalesmanModels.ToList()
                                  orderby calculateDistance(lat, lng, salesman.Latitude, salesman.Longitude)
                                  select new {
                                        salesman.FirstName,
                                        salesman.LastName,
                                        Distance = calculateDistance(lat, lng, salesman.Latitude, salesman.Longitude),
                                        salesman.Latitude,
                                        salesman.Longitude
                                  };
            return Json(modelByDistance, JsonRequestBehavior.AllowGet);
        }
